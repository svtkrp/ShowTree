...
import elemToStr from './elem_to_string.js';
...
export default function getGraphData(tree, m, edge) {
  ...
  // nodes and links
  nodes[0] = {id: rootIndex, val: elemToStr(TREE.get(rootIndex)), cx: 0, cy: y0, cz: 0, fx: 0, fy: y0, fz: 0};
  var i = 1;
  for (var level = 1; level < byLevels.length; level++) {
    ...
    for (var j = 0; j < byLevels[level].length; j++) {
      ...
      nodes[i] = {id: byLevels[level][j][1], val: elemToStr(TREE.get(byLevels[level][j][1])), cx: newcx, cy: newcy, cz: 0, fx: newcx, fy: newcy, fz: 0};
